...
const STORAGE_KEY = "theme-customizer-state"
...
export function ThemeCustomizer({ open, onOpenChange }: ThemeCustomizerProps) {
  ...
  const [selectedTheme, setSelectedTheme] = React.useState(persisted?.selectedTheme ?? "default")
  const [selectedTweakcnTheme, setSelectedTweakcnTheme] = React.useState(persisted?.selectedTweakcnTheme ?? "")
  const [selectedRadius, setSelectedRadius] = React.useState(persisted?.selectedRadius ?? "0.5rem")
  const [importModalOpen, setImportModalOpen] = React.useState(false)
  const [importedTheme, setImportedTheme] = React.useState<ImportedTheme | null>(persisted?.importedTheme ?? null)

  // Persist state whenever selections change
  React.useEffect(() => {
    persistState({ selectedTheme, selectedTweakcnTheme, selectedRadius, importedTheme })
  ...
  // Re-apply persisted/selected theme on mount and when mode changes
  React.useEffect(() => {
    if (importedTheme) {
  ...
  // Re-apply persisted radius on mount
  React.useEffect(() => {
    if (selectedRadius) {
      applyRadius(selectedRadius)
    }
  }, [selectedRadius, applyRadius])
